...
const buttons = Extra.markup(
    // markup creates buttons 
    // the api has a keyboard(needs resize) and inlinekeyboard
...
bot.start(async (ctx) => {
  ...
  await ctx.reply(`A contagem atual está em ${cont}`, buttons);
});

bot.action(/add (\d+)/, (ctx) => {
  cont += parseInt(ctx.match[1]);
  ctx.reply(`A contagem atual está em ${cont}`, buttons);
});
...
bot.action("result", (ctx) => {
  ctx.answerCbQuery(`A contagem atual está em ${cont}`);
});
